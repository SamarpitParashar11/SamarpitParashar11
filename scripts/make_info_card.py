...
ROWS = [
    ("host",),
    ("kv", "Now", "Developer & Researcher @ JSAC"),
    ("kv", "Prev", "Building ArogyaGrid (Smart Healthcare)"),
    ("kv", "Focus", "Full-Stack Development, ML & Computer Vision"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, JavaScript, TypeScript, C++"),
    ("kv", "Frontend", "React, Next.js, Three.js, Tailwind CSS"),
    ("kv", "Backend", "FastAPI, Flask, Node.js, Express"),
    ("kv", "AI / ML", "OpenCV, Scikit-Learn, PyTorch, LangChain"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Built ArogyaGrid & 20+ open source projects"),
    ("bul", "AI/ML developer passionate about space & health tech"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">samarpit@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">samarpit</tspan><tspan fill="{MUTED}">@</tspan>'
